...
{$O Sysop2o   } {$O Sysop5    } {$O SysOp12   } {$O OneLiner  } {$O WallPost  }
{$O Sysop2p   }

CONST
  NeedToHangUp  : Boolean = FALSE;
  NewRGVer      : String[20] = '2.0.x-dev';  { Semver (https://semver.org/) }
    ...
    IF ((SC(ParamStr(Counter),1) = '-') OR (SC(ParamStr(Counter),1) = '/')) THEN
      CASE SC(ParamStr(Counter),2) OF
        ...
        '1' : BEGIN
              ...
              Reset(NodeFile);
              Seek(NodeFile, 0);
              If Exist(StartDir + '\MISC\NL.ANS') Then
               Begin
              Exec(GetEnv('COMSPEC'), '/C TYPE MISC\NL.ANS');
               End
              Else
               Begin
                WriteLn;
              ...
              For m := 1 to FileSize(NodeFile) Do
               Begin
              ...
              If (NodeR.User > 0) Then
               Begin
                  ...
                  Write(NodeR.User,'  ');
                  TextColor(15);
                  Write(NodeR.UserName, '  ');
                  TextColor(8);
                  Write(NodeR.CityState, '  ');
                  TextColor(15);
                  Write(NodeR.Sex, '  ');
                  TextColor(8);
                  Write(NodeR.Age, '  ');
                  TextColor(7);
                  WriteLn(NodeR.ActivityDesc);
                 End;

                 Seek(NodeFIle,FilePos(NodeFile));
                End;
                TextColor(7);
                WriteLn;
              Close(NodeFile);
        ...
        'V' : Begin
               ...
               Write(' The Renegade Bulletin Board System v'+General.Version+' on ');
               ...
               CASE Tasker OF
                ...
                Win32    : Write('Win32 ');
                Dos5N    : Write('DOS/N ');
                FreeDOS  : Write('FreeDOS ');
               END;

               WriteLn('v',Lo(DosVersion),'.',Hi(DosVersion));
               TextColor(8);
               WriteLn(' Based on Renegade v1.19/Alpha By The Renegade Development Team');
               WriteLn(' Copyright (c) 1991-2013');
               TextColor(7);
               WriteLn;
...
BEGIN
  ...
  If General.Version <> NewRGVer Then
     Begin
      Seek(GeneralF, 0);
      General.Version := NewRGVer;
  ...
  OvrFileMode := 0;
  WriteLn;
  Write(' Initializing Renegade BBS v'+General.Version+' ... ');
  ...
  BEGIN
    ...
    TextColor(3);
    Write(' Attempting to load overlays into ');
    TextColor(11);
    Write('XMS');
    TextColor(3);
    Write(' memory ... ');
  ...
  WriteLn;
  WriteLn(' Exiting Renegade BBS with errorlevel ',ExitErrorLevel);
  WriteLn;
  Halt(ExitErrorLevel);
END.
