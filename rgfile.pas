...
BEGIN
  CLrScr;
  WriteLn('Renegade System Extra File String Compiler Version 1.0');
  Writeln('Copyright 2006 - The Renegade Developement Team');
  WriteLn;
  Write('Compiling extra file strings ... ');
  Found := TRUE;
  Assign(StrPointerFile,'RGFILEPR.DAT');
  ReWrite(StrPointerFile);
  Assign(RGStrFile,'RGFILETX.DAT');
  ReWrite(RGStrFile,1);
  Assign(F,'RGFILE.TXT');
  Reset(F);
  ...
  BEGIN
    ...
    BEGIN
      ...
      S := AllCaps(S);
      RGStrNum := -1;
      IF (S = 'CURRENT_CONF_CHOOSE') THEN
        RGStrNum := 0
      ELSE IF (S = 'JOIN_WHICH_CONF') THEN
        RGStrNum := 1
      ELSE IF (S = 'JOINED_CONF') THEN
        RGStrNum := 2
      ELSE IF (S = 'BATCH_MUST_DOWNLOAD') THEN
        RGStrNum := 3
      ELSE IF (S = 'BATCH_MUST_DOWNLOAD_PROMPT') THEN
        RGStrNum := 4
      ELSE IF (S = 'BATCH_MUST_UPLOAD') THEN
        RGStrNum := 5
      ELSE IF (S = 'BATCH_MUST_UPLOAD_PROMPT') THEN
        RGStrNum := 6;
