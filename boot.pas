...
BEGIN
  ...
  TextColor(3);
  (* Fun with colors!  Lets spice it up! Added 02/13/2013 {sk-5@bbs.godta.com} *)

  Write(' Opening and checking '); TextColor(11); Write('NODE'+IntToStr(ThisNode)+'.DAT '); TextColor(3); Write('... ');
  ...
  BEGIN
    ...
    BEGIN
      ...
      TextColor(3);
      Write('  Creating missing file ');
      TextColor(11);
      Write('NODE'+IntToStr(ThisNode)+'.DAT ');
      TextColor(3);
      Write('... ');
      ReWrite(LineFile);
    END
  ...
  SL1('');
  SL1('');
  SL1('|03Renegade |11'+General.Version+' |03node |11'+IntToStr(ThisNode)+' |03loaded on |11'+dat+'|03 ...');
  ...
  BEGIN
    IF (LastError = 0) THEN
      Close(UserIDXFile);
    TextColor(4);
    Write(' Regenerating corrupted user index :   ');
    TextColor(15);
    Write('0%');
...
BEGIN
  ...
  ELSE IF (WinNTOk) THEN
    Tasker := Win32
  ELSE IF (Lo(DosVersion) >= 7) AND (Hi(DosVersion) >= 10) THEN
    Tasker := FreeDOS
  ELSE IF (D5 = 5) THEN
    Tasker := Dos5N;
...
BEGIN
  ...
  BEGIN
    ...
    Write(' Share ');
    TextColor(3);
    WriteLn('should be loaded for multinode operation.');
    Delay(1000);
  END;
  ...
  BEGIN
    ...
    TextColor(3);
    WriteLn;
    Write(' No node number specified.  defaulting to node ');
    TextColor(11);
    Write('1');
    TextColor(3);
    WriteLn('.');
    ThisNode := 1; {For godta bbs, if anyone uses this remember to change to 1}
    Delay(1000);
  END
